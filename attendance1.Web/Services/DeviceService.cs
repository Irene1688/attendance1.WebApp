    ...
    public class DeviceService
    {
        ...
        public async Task<StudentDeviceMdl> GetDeviceInfoAsync(string deviceInfo)
        {
            try
            {
                ...
                //var deviceDetector = new DeviceDetector(userAgent);
                //deviceDetector.Parse();
                //string deviceOSDetail = deviceDetector.GetOs().ToString();
                //string pattern = @"Name:\s*(\w+)";
                //Match match = Regex.Match(deviceOSDetail, pattern);
                //if (match.Success)
                //{
                //    deviceOS = match.Groups[1].Value;
                //}
                ////if (deviceOS == "AND" || deviceOS == "WIN")
                ////{
                ////ipaddress = 
                ////}
        ...
        //    {
        //        // The X-Forwarded-For header can contain multiple IP addresses, split them by comma
        ...
        #region 原版的mac address处理
        //public async Task<string> ValidateStudentDeviceAndGetIdAsync(string studentID, StudentDeviceMdl studentCurrentDevice)
        //{
        //    try
        //    {
